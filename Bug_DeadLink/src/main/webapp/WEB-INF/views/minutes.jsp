...
<div class="content">
	...
	<div class="restul_group fl_no" style="width:1200px;">
				...
				<ul>
					...
					<li><span>구분</span>
						<select name="cate" id="select02" class="sel_type1" title="구분">
									...
									<option value="minutes" selected="selected">회의록</option>
			...
			<div class="result_tit mt_40">회의록<span>(${totalsize}건)</span>
						<ul class="sorting">
						<c:choose>
						<c:when test="${logtype eq 'recentcollect'}">
									<li><a href="#" <c:if test="${sorting eq 'correct'}">
    								<c:out value="class=on" />
									</c:if> onclick="MovingPage('1', 'minutes', '${logtype}', '10', 'correct')">지역순</a><span></span></li>
									<li><a href="#" <c:if test="${sorting eq 'recent'}">
    								<c:out value="class=on" />
									</c:if> onclick="MovingPage('1', 'minutes', '${logtype}', '10', 'recent')">상태순</a><span></span></li>
						</c:when>
						
						<c:otherwise>
									<li><a href="#" <c:if test="${sorting eq 'correct'}">
    								<c:out value="class=on" />
									</c:if> onclick="MovingPage('1', 'minutes', '${logtype}', '10', 'correct')">정확도순</a><span></span></li>
									<li><a href="#" <c:if test="${sorting eq 'recent'}">
    								<c:out value="class=on" />
									</c:if> onclick="MovingPage('1', 'minutes', '${logtype}', '10', 'recent')">개수순</a><span></span></li>
									<li>
